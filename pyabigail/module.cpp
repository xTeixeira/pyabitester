...
std::vector<char **>
convert_to_char_ptr_vector(const std::vector<std::string> &strings) {
...
std::shared_ptr<abigail::elf::reader> wrapped_create_best_elf_based_reader(
    ...
    const std::vector<std::string> &debug_info_root_paths,
    abigail::ir::environment &env,
    abigail::corpus::origin requested_debug_info_kind, bool show_all_types,
  ...
  auto result = abigail::tools_utils::create_best_elf_based_reader(
      elf_file_path, converted_paths, env, requested_debug_info_kind,
...
PYBIND11_MODULE(pyabigail, m) {
  ...
  // TODO: py::nodelete prevents destructors from being called the
  // permanent solution will be to expose the abigail::elf::reader
  // class as a Python context manager
  py::class_<abigail::elf::reader,
             std::unique_ptr<abigail::elf::reader, py::nodelete>>(m, "Reader")
      .def("read_corpus", &abigail::elf::reader::read_corpus);

  py::enum_<abigail::fe_iface::status>(m, "Status")
      .value("ALT_DEBUG_INFO_NOT_FOUND",
             abigail::fe_iface::STATUS_ALT_DEBUG_INFO_NOT_FOUND)
      .value("DEBUG_INFO_NOT_FOUND",
             abigail::fe_iface::STATUS_DEBUG_INFO_NOT_FOUND)
  ...
  py::enum_<abigail::corpus::origin>(m, "Origin")
      ...
      .value("LINUX_KERNEL_BINARY",
             abigail::corpus::origin::LINUX_KERNEL_BINARY_ORIGIN)
      .value("NATIVE_XML", abigail::corpus::origin::NATIVE_XML_ORIGIN);

  m.def("create_best_elf_based_reader", &wrapped_create_best_elf_based_reader,
        py::arg("elf_file_path"), py::arg("debug_info_root_paths"),
        py::arg("env"), py::arg("requested_debug_info_kind"),
